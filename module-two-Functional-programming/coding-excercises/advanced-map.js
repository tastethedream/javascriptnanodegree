// One common thing we use map for in real life is to reformat objects to have a shape 
// that is better for our purposes. For instance, we only care about the 
...
const newArray = index.map((raceCode) => {
    return {name: raceCode.value, firstThree: raceCode.id.substring(0,3)}
});
